...
manage both sets of drones from the same controller, whilst having control over which set of drones
an individual backend is run on.
...
Clusters are identified by a name, which doubles as the hostname through which clients will connect
to backends belonging to that cluster.

...
use crate::{
    names::BackendName,
    types::{BackendStatus, ClusterId, TimestampedBackendStatus},
};
use axum::{
    extract::{Path, State},
    response::{
...
pub async fn handle_backend_status(
    ...
    let status = backend_status(&controller, &cluster_id, &backend_id).await?;
    Ok(Json(status))
}

pub async fn handle_backend_status_stream(
    Path((_cluster_id, backend_id)): Path<(ClusterId, BackendName)>, // TODO: check cluster id
    State(controller): State<Controller>,
    headers: HeaderMap,
